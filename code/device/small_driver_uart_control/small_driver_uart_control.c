...
//-------------------------------------------------------------------------------------------------------------------
void uart_control_callback(uint8 uart_index) {
    uint8 receive_data;  // 定义临时变量

    if (uart_query_byte(uart_index, &receive_data))  // 接收串口数据
    {
        if (receive_data == 0xA5 &&
            motor_value.receive_data_buffer[0] !=
                0xA5)  // 判断是否收到帧头 并且 当前接收内容中是否正确包含帧头
        {
            motor_value.receive_data_count =
                0;  // 未收到帧头或者未正确包含帧头则重新接收
        }

        motor_value.receive_data_buffer[motor_value.receive_data_count++] =
            receive_data;  // 保存串口数据
...
void small_driver_uart_init(void) {
    uart_init(SMALL_DRIVER_UART_LEFT, SMALL_DRIVER_BAUDRATE,
              SMALL_DRIVER_RX_LEFT, SMALL_DRIVER_TX_LEFT);  // 串口初始化
    uart_init(SMALL_DRIVER_UART_RIGHT, SMALL_DRIVER_BAUDRATE,
              SMALL_DRIVER_RX_RIGHT, SMALL_DRIVER_TX_RIGHT);  // 串口初始化
