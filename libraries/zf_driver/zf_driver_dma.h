...
#define clear_dma_flag(dma_ch) \
    (IfxDma_clearChannelInterrupt(&MODULE_DMA, dma_ch))

#define dma_set_destination(dma_ch, destination_addr) \
    (IfxDma_setChannelDestinationAddress(             \
        &MODULE_DMA, (dma_ch),                        \
        (void*)IFXCPU_GLB_ADDR_DSPR(IfxCpu_getCoreId(), (destination_addr))))
