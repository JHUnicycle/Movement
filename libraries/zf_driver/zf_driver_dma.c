...
// 返回参数      uint8
// 使用示例      dma_init(MT9V03X_DMA_CH, MT9V03X_DATA_ADD, mt9v03x_image[0],
// MT9V03X_PCLK_PIN, EXTI_TRIGGER_RISING, MT9V03X_IMAGE_SIZE); 备注信息
//-------------------------------------------------------------------------------------------------------------------
uint8 dma_init(IfxDma_ChannelId dma_ch,
               uint8* source_addr,
               uint8* destination_addr,
               exti_pin_enum exti_pin,
               exti_trigger_enum trigger,
               uint32 dma_count,
               uint8 isr_prio) {
    IfxDma_Dma_Channel dmaChn;

    exti_init(exti_pin, trigger);  // eru触发DMA通道号
                                   // 在eru文件中设置eru的优先级，即为触发的通道
    ...
    if (16384 < single_channel_dma_count) {
        while (TRUE) {
            single_channel_dma_count = dma_count / list_num;
            if ((single_channel_dma_count <= 16384) &&
                !(dma_count % list_num)) {
                break;
            }
            list_num++;
    ...
    if (1 == list_num) {
        ...
        cfg.shadowAddress = IFXCPU_GLB_ADDR_DSPR(
            IfxCpu_getCoreId(), (unsigned)&dma_link_list.linked_list[1]);
    }
    ...
    if (1 < list_num) {
        i = 0;
        while (i < list_num) {
            cfg.destinationAddress = IFXCPU_GLB_ADDR_DSPR(
                IfxCpu_getCoreId(),
                destination_addr + single_channel_dma_count * i);
            if (i == (list_num - 1)) {
                cfg.shadowAddress = IFXCPU_GLB_ADDR_DSPR(
                    IfxCpu_getCoreId(), (uint32)&dma_link_list.linked_list[0]);
            } else {
                cfg.shadowAddress = IFXCPU_GLB_ADDR_DSPR(
                    IfxCpu_getCoreId(),
                    (uint32)&dma_link_list.linked_list[i + 1]);
            }
            cfg.transferCount = (uint16)single_channel_dma_count;
...
// MT9V03X_PCLK_PIN, EXTI_TRIGGER_RISING, MT9V03X_IMAGE_SIZE); 备注信息
//-------------------------------------------------------------------------------------------------------------------
uint8 dma_init_2(IfxDma_ChannelId dma_ch,
                 uint8* source_addr,
                 uint8* destination_addr,
                 exti_pin_enum exti_pin,
                 exti_trigger_enum trigger,
    ...
    if (1 == list_num) {
        ...
        cfg.shadowAddress = IFXCPU_GLB_ADDR_DSPR(
            IfxCpu_getCoreId(), (unsigned)&dma_link_list_2.linked_list[1]);
    }
    ...
    if (1 < list_num) {
        ...
        while (i < list_num) {
            ...
            if (i == (list_num - 1)) {
                cfg.shadowAddress = IFXCPU_GLB_ADDR_DSPR(
                    IfxCpu_getCoreId(),
                    (uint32)&dma_link_list_2.linked_list[0]);

            } else {
                cfg.shadowAddress = IFXCPU_GLB_ADDR_DSPR(
                    IfxCpu_getCoreId(),
                    (uint32)&dma_link_list_2.linked_list[i + 1]);
            }
            cfg.transferCount = (uint16)single_channel_dma_count;

            IfxDma_Dma_initLinkedListEntry(
                (void*)&dma_link_list_2.linked_list[i], &cfg);
...
//-------------------------------------------------------------------------------------------------------------------
// 函数简介     dma 传输禁止
// 参数说明     ch              选择 dma 通道 (详见 zf_driver_dma.h 中枚举
// dma_channel_enum 定义) 返回参数     void 使用示例
// dma_disable(MT9V03X_DMA_CH); 备注信息
//-------------------------------------------------------------------------------------------------------------------
...
// 函数简介     dma 传输使能
// 参数说明     ch              选择 dma 通道 (详见 zf_driver_dma.h 中枚举
// dma_channel_enum 定义) 返回参数     void 使用示例 dma_enable(MT9V03X_DMA_CH);
// 备注信息
//-------------------------------------------------------------------------------------------------------------------
