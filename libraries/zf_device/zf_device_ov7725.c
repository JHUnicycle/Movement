 ...
 *                  ------------------------------------
 *                  模块管脚             单片机管脚
 *                  TXD/SCL            查看 zf_device_ov7725.h 中
 *OV7725_COF_UART_TX 或 OV7725_COF_IIC_SCL 宏定义 RXD/SDA            查看
 *zf_device_ov7725.h 中 OV7725_COF_UART_RX 或 OV7725_COF_IIC_SDA 宏定义 PCLK
 *查看 zf_device_ov7725.h 中 OV7725_PCLK_PIN 宏定义 VSY                查看
 *zf_device_ov7725.h 中 OV7725_VSYNC_PIN 宏定义 D0-D7              查看
 *zf_device_ov7725.h 中 OV7725_DATA_PIN 宏定义 从该定义开始的连续八个引脚 VCC
 *3.3V电源 GND                电源地 其余引脚悬空
 *                  ------------------------------------
 ********************************************************************************************************************/
...
static uint8 ov7725_set_config(uint16 buff[OV7725_CONFIG_FINISH][2]) {
    ...
    do {
        if (3 <= fifo_used(&camera_receiver_fifo)) {
            uart_buffer_index = 3;
            fifo_read_buffer(&camera_receiver_fifo, uart_buffer,
                             &uart_buffer_index, FIFO_READ_AND_CLEAN);
...
static uint8 ov7725_iic_init(void) {
    ...
    soft_iic_init(&ov7725_iic_struct, OV7725_DEV_ADD, OV7725_COF_IIC_DELAY,
                  OV7725_COF_IIC_SCL, OV7725_COF_IIC_SDA);

    soft_iic_sccb_write_register(&ov7725_iic_struct, OV7725_COM7,
                                 0x80);  // 复位摄像头
    system_delay_ms(50);

    do {
        ov7725_idcode =
            soft_iic_sccb_read_register(&ov7725_iic_struct, OV7725_VER);
        ...
        soft_iic_sccb_write_register(
            &ov7725_iic_struct, OV7725_COM8,
            0xCE);  // 0xCE:关闭自动曝光  0xCF：开启自动曝光
        ...
        soft_iic_sccb_write_register(&ov7725_iic_struct, OV7725_AWB_Ctrl0,
                                     0xE0);
        soft_iic_sccb_write_register(&ov7725_iic_struct, OV7725_DSPAuto, 0xff);
        soft_iic_sccb_write_register(&ov7725_iic_struct, OV7725_DSP_Ctrl2,
                                     0x0C);
        soft_iic_sccb_write_register(&ov7725_iic_struct, OV7725_DSP_Ctrl3,
                                     0x00);
        soft_iic_sccb_write_register(&ov7725_iic_struct, OV7725_DSP_Ctrl4,
                                     0x00);

        if (OV7725_W == 80) {
            soft_iic_sccb_write_register(&ov7725_iic_struct, OV7725_HOutSize,
                                         0x14);
        ...
        if (OV7725_H == 60) {
            soft_iic_sccb_write_register(&ov7725_iic_struct, OV7725_VOutSize,
                                         0x1E);
...
static void ov7725_vsync_handler(void) {
    ...
    if ((ov7725_uart_dma_init_flag || ov7725_lost_flag) &&
        camera_type == CAMERA_BIN_UART) {
...
static void ov7725_dma_handler(void) {
    ...
    if (IfxDma_getChannelTransactionRequestLost(&MODULE_DMA,
                                                OV7725_DMA_CH)) {  // 图像有错位
...
uint8 ov7725_init(void) {
    ...
    do {
        ...
        set_camera_type(CAMERA_BIN_UART, ov7725_vsync_handler,
                        ov7725_dma_handler, ov7725_uart_handler);
        camera_fifo_init();

        uart_init(OV7725_COF_UART, OV7725_COF_BAUR, OV7725_COF_UART_RX,
                  OV7725_COF_UART_TX);
        ...
        if (ov7725_get_config(ov7725_get_confing_buffer)) {
            uart_rx_interrupt(OV7725_COF_UART, 0);
            system_delay_ms(200);
            set_camera_type(CAMERA_BIN_IIC, ov7725_vsync_handler,
                            ov7725_dma_handler, NULL);  // 设置连接摄像头类型
            if (ov7725_iic_init()) {
        ...
        ov7725_link_list_num = camera_init(
            OV7725_DATA_ADD, ov7725_image_binary[0], OV7725_IMAGE_SIZE);
    } while (0);
    return return_state;
}
