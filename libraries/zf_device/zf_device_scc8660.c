 ...
 *                  ------------------------------------
 *                  模块管脚             单片机管脚
 *                  TXD                查看 zf_device_scc8660.h 中
 *SCC8660_COF_UART_TX 宏定义 RXD                查看 zf_device_scc8660.h 中
 *SCC8660_COF_UART_RX 宏定义 PCLK               查看 zf_device_scc8660.h 中
 *SCC8660_PCLK_PIN 宏定义 VSY                查看 zf_device_scc8660.h 中
 *SCC8660_VSYNC_PIN 宏定义 D0-D7              查看 zf_device_scc8660.h 中
 *SCC8660_DATA_PIN 宏定义 从该定义开始的连续八个引脚 VCC                3.3V电源
 *                  GND                电源地
 *                  其余引脚悬空
 *                  ------------------------------------
 ********************************************************************************************************************/
...
static int16 scc8660_get_confing_buffer[SCC8660_CONFIG_FINISH - 1][2] = {
    {SCC8660_AUTO_EXP, 0},    {SCC8660_BRIGHT, 0},  // 亮度设置
    {SCC8660_FPS, 0},                               // 图像帧率
...
static uint8 scc8660_set_config(int16 buff[SCC8660_CONFIG_FINISH][2]) {
    ...
    // 设置参数  具体请参看问题锦集手册
    // 开始配置摄像头并重新初始化
    for (loop_count = SCC8660_MANUAL_WB; loop_count < SCC8660_SET_REG_DATA;
         loop_count--) {
        uart_buffer[0] = 0xA5;
...
// 函数简介     单独设置摄像头曝光时间
// 参数说明     light
// 设置曝光时间越大图像越亮，摄像头收到后会根据分辨率及FPS计算最大曝光时间如果设置的数据过大，那么摄像头将会设置这个最大值
// 返回参数     uint16          数据
    ...
    if (scc8660_dma_init_flag || scc8660_lost_flag) {
        ...
        if (1 == scc8660_link_list_num) {
            dma_set_destination(
                SCC8660_DMA_CH,
                (uint8*)
                    scc8660_image[0]);  // 没有采用链接传输模式 重新设置目的地址
        }
        dma_enable(SCC8660_DMA_CH);
    }
    scc8660_lost_flag = 1;
...
static void scc8660_dma_handler(void) {
    ...
    if (IfxDma_getChannelTransactionRequestLost(
            &MODULE_DMA, SCC8660_DMA_CH))  // 图像错位判断
    {
...
uint8 scc8660_init(void) {
    ...
    do {
        system_delay_ms(200);
        set_camera_type(CAMERA_COLOR, scc8660_vsync_handler,
                        scc8660_dma_handler, NULL);  // 设置连接摄像头类型
        // 首先尝试SCCB通讯
        scc8660_type = SCC8660_SCCB;
        soft_iic_init(&scc8660_iic_struct, 0, SCC8660_COF_IIC_DELAY,
                      SCC8660_COF_IIC_SCL, SCC8660_COF_IIC_SDA);
        if (scc8660_set_config_sccb(&scc8660_iic_struct,
                                    scc8660_set_confing_buffer)) {
            // SCCB通讯失败，尝试串口通讯
            scc8660_type = SCC8660_UART;
            camera_fifo_init();
            set_camera_type(CAMERA_COLOR, scc8660_vsync_handler,
                            scc8660_dma_handler,
                            &scc8660_uart_callback);  // 设置连接摄像头类型
            uart_init(SCC8660_COF_UART, SCC8660_COF_BAUR, SCC8660_COF_UART_RX,
                      SCC8660_COF_UART_TX);  // 初始换串口 配置摄像头
        ...
        scc8660_link_list_num = camera_init(
            SCC8660_DATA_ADD, (uint8*)scc8660_image[0], SCC8660_IMAGE_SIZE);
    } while (0);
