...
IFX_ALIGN(4)
uint8 mt9v03x2_image[MT9V03X2_H][MT9V03X2_W]; // 必须4字节对齐

static m9v03x2_type_enum mt9v03x2_type; // 定义摄像头类型
static uint16 mt9v03x2_version = 0x00;  // 定义摄像头版本号
...
static int16 mt9v03x2_get_confing_buffer[MT9V03X2_CONFIG_FINISH - 1][2] =
    {
        {MT9V03X2_AUTO_EXP, 0},  // 自动曝光设置
        {MT9V03X2_EXP_TIME, 0},  // 曝光时间
        {MT9V03X2_FPS, 0},       // 图像帧率
        {MT9V03X2_SET_COL, 0},   // 图像列数量
        {MT9V03X2_SET_ROW, 0},   // 图像行数量
        {MT9V03X2_LR_OFFSET, 0}, // 图像左右偏移量
        {MT9V03X2_UD_OFFSET, 0}, // 图像上下偏移量
        {MT9V03X2_GAIN, 0},      // 图像增益
        {MT9V03X2_PCLK_MODE, 0}, // 像素时钟模式命令 PCLK模式 < 仅总钻风 MT9V034 V1.5 以及以上版本支持该命令 >
};
...
// 使用示例     mt9v03x2_set_config(mt9v03x2_set_confing_buffer);
// 备注信息     内部调用
//-------------------------------------------------------------------------------------------------------------------
...
// 参数说明     buff            接收配置信息的地址
// 返回参数     uint8           1-失败 0-成功
// 使用示例     mt9v03x2_get_config(mt9v03x2_get_confing_buffer);
// 备注信息     内部调用
//-------------------------------------------------------------------------------------------------------------------
static uint8 mt9v03x2_get_config(int16 buff[MT9V03X2_CONFIG_FINISH - 1][2])
{
    ...
    for (loop_count = loop_count - 1; 1 <= loop_count; loop_count--)
    {
        ...
        uart_buffer[0] = 0xA5;
        uart_buffer[1] = MT9V03X2_GET_STATUS;
        ...
        if (MT9V03X2_INIT_TIMEOUT < timeout_count) // 超时
        {
            return_state = 1;
            break;
        }
...
//-------------------------------------------------------------------------------------------------------------------
//  函数简介      MT9V03X2摄像头串口通信回调
//  参数说明      void
//  返回参数      void
//  使用示例      mt9v03x2_uart_callback();
//-------------------------------------------------------------------------------------------------------------------
...
static void mt9v03x2_dma_handler(void)
{
    ...
    if (IfxDma_getChannelTransactionRequestLost(&MODULE_DMA, MT9V03X2_DMA_CH)) // 图像错位判断
    {
        ...
        IfxDma_clearChannelTransactionRequestLost(&MODULE_DMA, MT9V03X2_DMA_CH);
...
// 使用示例     mt9v03x2_get_version();                          // 调用该函数前请先初始化串口
// 备注信息
//-------------------------------------------------------------------------------------------------------------------
...
// 参数说明     light           设定曝光时间
// 返回参数     uint8           1-失败 0-成功
// 使用示例     mt9v03x2_set_exposure_time(100);                 // 调用该函数前请先初始化串口
// 备注信息     设置曝光时间越大图像越亮
...
uint8 mt9v03x2_set_exposure_time(uint16 light)
{
    ...
    if (MT9V03X2_UART == mt9v03x2_type)
    {
        ...
        uart_buffer[0] = 0xA5;
        uart_buffer[1] = MT9V03X2_SET_EXP_TIME;
        temp = light;
        uart_buffer[2] = temp >> 8;
        uart_buffer[3] = (uint8)temp;
        uart_write_buffer(MT9V03X2_COF_UART, uart_buffer, 4);
        ...
        set_camera_type(CAMERA_GRAYSCALE2, mt9v03x2_vsync_handler, mt9v03x2_dma_handler, NULL);
    }
    else
    {
...
// 参数说明     data            需要写入的数据
// 返回参数     uint8           1-失败 0-成功
// 使用示例     mt9v03x2_set_reg(addr, data);                    // 调用该函数前请先初始化串口
// 备注信息
//-------------------------------------------------------------------------------------------------------------------
uint8 mt9v03x2_set_reg(uint8 addr, uint16 data)
{
    ...
    if (MT9V03X2_UART == mt9v03x2_type)
    {
        ...
        uart_buffer[0] = 0xA5;
        uart_buffer[1] = MT9V03X2_SET_ADDR;
        temp = addr;
        uart_buffer[2] = temp >> 8;
        uart_buffer[3] = (uint8)temp;
        uart_write_buffer(MT9V03X2_COF_UART, uart_buffer, 4);
        ...
        system_delay_ms(10);
        uart_buffer[0] = 0xA5;
        uart_buffer[1] = MT9V03X2_SET_DATA;
        temp = data;
        uart_buffer[2] = temp >> 8;
        uart_buffer[3] = (uint8)temp;
        uart_write_buffer(MT9V03X2_COF_UART, uart_buffer, 4);
...
uint8 mt9v03x2_init(void)
{
    ...
    do
    {
        ...
        if (mt9v03x_set_config_sccb(&mt9v03x2_iic_struct, mt9v03x2_set_confing_buffer))
        {
            ...
            //     break;
            // }
            mt9v03x2_version = mt9v03x2_get_version(); // 获取配置的方式
