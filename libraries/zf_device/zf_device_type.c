 ...
 * 2024-01-16       pudding            移除SPI WIFI 中断回调指针 SPI
 *WIFI将不再使用外部中断
 ********************************************************************************************************************/
...
callback_function camera_uart_handler =
    type_default_callback;  // 串口通讯中断函数指针，根据初始化时设置的函数进行跳转
callback_function camera_vsync_handler =
    type_default_callback;  // 场中断函数指针，根据初始化时设置的函数进行跳转
callback_function camera_dma_handler =
    type_default_callback;  // DMA完成中断函数指针，根据初始化时设置的函数进行跳转

camera_type_enum camera_type_2 = NO_CAMERE;  // 摄像头类型变量
callback_function camera_uart_handler_2 =
    type_default_callback;  // 串口通讯中断函数指针，根据初始化时设置的函数进行跳转
callback_function camera_vsync_handler_2 =
    type_default_callback;  // 场中断函数指针，根据初始化时设置的函数进行跳转
callback_function camera_dma_handler_2 =
    type_default_callback;  // DMA完成中断函数指针，根据初始化时设置的函数进行跳转

wireless_type_enum wireless_type = NO_WIRELESS;
callback_function wireless_module_uart_handler =
    type_default_callback;  // 无线串口接收中断函数指针，根据初始化时设置的函数进行跳转

tof_type_enum tof_type = NO_TOF;
callback_function tof_module_exti_handler =
    type_default_callback;  // ToF 模块 INT 更新中断
...
void set_camera_type(camera_type_enum type_set,
                     callback_function vsync_callback,
                     callback_function dma_callback,
                     callback_function uart_callback) {
    camera_type = type_set;
    camera_uart_handler =
        ((uart_callback == NULL) ? (type_default_callback) : (uart_callback));
    camera_vsync_handler =
        ((vsync_callback == NULL) ? (type_default_callback) : (vsync_callback));
    camera_dma_handler =
        ((dma_callback == NULL) ? (type_default_callback) : (dma_callback));
}
...
// 备注信息     一般由各摄像头初始化内部调用
//-------------------------------------------------------------------------------------------------------------------
void set_camera_type_2(camera_type_enum type_set,
                       callback_function vsync_callback,
                       callback_function dma_callback,
                       callback_function uart_callback) {
    camera_type_2 = type_set;
    camera_uart_handler_2 =
        ((uart_callback == NULL) ? (type_default_callback) : (uart_callback));
...
void set_wireless_type(wireless_type_enum type_set,
                       callback_function wireless_callback) {
    wireless_type = type_set;
    wireless_module_uart_handler =
        ((wireless_callback == NULL) ? (type_default_callback)
                                     : (wireless_callback));
}
...
void set_tof_type(tof_type_enum type_set, callback_function exti_callback) {
    ...
    tof_module_exti_handler =
        ((exti_callback == NULL) ? (type_default_callback) : (exti_callback));
}
