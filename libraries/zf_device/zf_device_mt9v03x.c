 ...
 *                  ------------------------------------
 *                  模块管脚             单片机管脚
 *                  TXD                查看 zf_device_mt9v03x.h 中
 *MT9V03X_COF_UART_TX 宏定义 RXD                查看 zf_device_mt9v03x.h 中
 *MT9V03X_COF_UART_RX 宏定义 PCLK               查看 zf_device_mt9v03x.h 中
 *MT9V03X_PCLK_PIN 宏定义 VSY                查看 zf_device_mt9v03x.h 中
 *MT9V03X_VSYNC_PIN 宏定义 D0-D7              查看 zf_device_mt9v03x.h 中
 *MT9V03X_DATA_PIN 宏定义 从该定义开始的连续八个引脚 VCC                3.3V电源
 *                  GND                电源地
 *                  其余引脚悬空
 *                  ------------------------------------
 ********************************************************************************************************************/
...
static int16 mt9v03x_set_confing_buffer[MT9V03X_CONFIG_FINISH][2] = {
    ...
    {MT9V03X_AUTO_EXP,
     MT9V03X_AUTO_EXP_DEF}, // 自动曝光设置   范围1-63 0为关闭 如果自动曝光开启
                            // EXP_TIME命令设置的数据将会变为最大曝光时间，也就是自动曝光时间的上限
    {MT9V03X_EXP_TIME,
     MT9V03X_EXP_TIME_DEF}, // 曝光时间
                            // 摄像头收到后会自动计算出最大曝光时间，如果设置过大则设置为计算出来的最大曝光值
    {MT9V03X_FPS,
     MT9V03X_FPS_DEF},            // 图像帧率
                                  // 摄像头收到后会自动计算出最大FPS，如果过大则设置为计算出来的最大FPS
    {MT9V03X_SET_COL, MT9V03X_W}, // 图像列数量    范围1-752
    {MT9V03X_SET_ROW, MT9V03X_H}, // 图像行数量    范围1-480
    {MT9V03X_LR_OFFSET,
     MT9V03X_LR_OFFSET_DEF}, // 图像左右偏移量  正值 右偏移   负值 左偏移
                             // 列为188 376 752时无法设置偏移
                             // 摄像头收偏移数据后会自动计算最大偏移，如果超出则设置计算出来的最大偏移
    {MT9V03X_UD_OFFSET,
     MT9V03X_UD_OFFSET_DEF}, // 图像上下偏移量  正值 上偏移   负值 下偏移
                             // 行为120 240 480时无法设置偏移
                             // 摄像头收偏移数据后会自动计算最大偏移，如果超出则设置计算出来的最大偏移
    {MT9V03X_GAIN,
     MT9V03X_GAIN_DEF},                         // 图像增益      范围16-64
                                                // 增益可以在曝光时间固定的情况下改变图像亮暗程度
    {MT9V03X_PCLK_MODE, MT9V03X_PCLK_MODE_DEF}, // 像素时钟模式 仅总钻风MT9V034
                                                // V2.0以及以上版本支持该命令
};
...
static int16 mt9v03x_get_confing_buffer[MT9V03X_CONFIG_FINISH - 1][2] = {
    ...
    {MT9V03X_GAIN, 0},      // 图像增益
    {MT9V03X_PCLK_MODE, 0}, // 像素时钟模式命令 PCLK模式 < 仅总钻风 MT9V034
                            // V1.5 以及以上版本支持该命令 >
};
...
static uint8 mt9v03x_get_config(int16 buff[MT9V03X_CONFIG_FINISH - 1][2])
{
    ...
    {
        if ((0x0230 > mt9v03x_version) &&
            (MT9V03X_PCLK_MODE == buff[loop_count][0]))
        {
            continue;
        }
        uart_buffer[0] = 0xA5;
...
static void mt9v03x_vsync_handler(void)
{
    ...
    else
    {
        if (1 == mt9v03x_link_list_num)
        {
            dma_set_destination(
                MT9V03X_DMA_CH,
                mt9v03x_image[0]); // 没有采用链接传输模式 重新设置目的地址
        }
        dma_enable(MT9V03X_DMA_CH);
    }
    mt9v03x_lost_flag = 1;
...
static void mt9v03x_dma_handler(void)
{
    ...
    if (IfxDma_getChannelTransactionRequestLost(
            &MODULE_DMA, MT9V03X_DMA_CH)) // 图像错位判断
    {
...
// 参数说明     light           设定曝光时间
// 返回参数     uint8           1-失败 0-成功
// 使用示例     mt9v03x_set_exposure_time(100);                 //
// 调用该函数前请先初始化串口 备注信息     设置曝光时间越大图像越亮
...
uint8 mt9v03x_set_exposure_time(uint16 light)
{
    ...
    {
        set_camera_type(CAMERA_GRAYSCALE, mt9v03x_vsync_handler,
                        mt9v03x_dma_handler, mt9v03x_uart_handler);
        uint8 uart_buffer[4];
        uint16 temp = 0;
        uint16 timeout_count = 0;
        uint32 uart_buffer_index = 0;
...
// 参数说明     data            需要写入的数据
// 返回参数     uint8           1-失败 0-成功
// 使用示例     mt9v03x_set_reg(addr, data);                    //
// 调用该函数前请先初始化串口 备注信息
//-------------------------------------------------------------------------------------------------------------------
...
uint8 mt9v03x_init(void)
{
    ...
    do
    {
        ...
        set_camera_type(CAMERA_GRAYSCALE, mt9v03x_vsync_handler,
                        mt9v03x_dma_handler, NULL);
        soft_iic_init(&mt9v03x_iic_struct, 0, MT9V03X_COF_IIC_DELAY,
                      MT9V03X_COF_IIC_SCL, MT9V03X_COF_IIC_SDA);
        if (mt9v03x_set_config_sccb(&mt9v03x_iic_struct,
                                    mt9v03x_set_confing_buffer))
        {
            ...
            // 初始换串口 配置摄像头
            uart_init(MT9V03X_COF_UART, MT9V03X_COF_BAUR, MT9V03X_COF_UART_RX,
                      MT9V03X_COF_UART_TX);
            uart_rx_interrupt(MT9V03X_COF_UART, 1);
            fifo_clear(&camera_receiver_fifo);
            // 等待摄像头上电初始化成功 方式有两种：延时或者通过获取配置的方式
            // 二选一 system_delay_ms(1000); // 延时方式
        ...
        mt9v03x_link_list_num =
            camera_init(MT9V03X_DATA_ADD, mt9v03x_image[0], MT9V03X_IMAGE_SIZE);
    } while (0);
    return return_state;
}
