...
#define SCC8660_COF_UART_TX \
    (UART1_RX_P02_3)  // 凌瞳 UART-TX 引脚 要接在单片机 RX 上
#define SCC8660_COF_UART_RX \
    (UART1_TX_P02_2)  // 凌瞳 UART-RX 引脚 要接在单片机 TX 上
...
#define SCC8660_PCLK_PIN \
    (ERU_CH2_REQ14_P02_1)  // PCLK 触发信号 TIM_ETR 引脚禁止随意修改
#define SCC8660_VSYNC_PIN (ERU_CH3_REQ6_P02_0)  // 场中断引脚
#define SCC8660_DATA_PIN \
...
#define SCC8660_IMAGE_SIZE \
    (SCC8660_W * 2 *       \
     SCC8660_H)  // 整体图像大小 SCC8660_W*2*SCC8660_H 不能超过 65535

#define SCC8660_AUTO_EXP_DEF \
    (0)  // 自动曝光     默认不开启自动曝光设置  范围 [0-1] 0为关闭
#define SCC8660_BRIGHT_DEF \
    (500)  // 亮度设置     手动曝光默认：300   手动曝光时：参数范围0-65535
           // 自动曝光推荐值：100 自动曝光时参数设置范围0-255
#define SCC8660_FPS_DEF \
    (60)  // 图像帧率     默认：50        可选参数为：60 50 30 25。
          // 实际帧率还需要看SCC8660_PCLK_DIV参数的设置
#define SCC8660_PCLK_DIV_DEF \
    (2)  // PCLK分频系数 默认：5         可选参数为：<0:1/1> <1:2/3> <2:1/2>
         // <3:1/3> <4:1/4> <5:1/8>
...
#define SCC8660_PCLK_MODE_DEF \
    (0)  // PCLK模式     默认：0         可选参数为：[0,1] 0：不输出消隐信号
         // 1：输出消隐信号
         // <通常都设置为0，如果使用STM32的DCMI接口采集需要设置为1>
#define SCC8660_COLOR_MODE_DEF \
    (0)  // 图像色彩模式 默认：0         可选参数为：[0,1] 0：正常彩色模式
         // 1：鲜艳模式（色彩饱和度提高）
#define SCC8660_DATA_FORMAT_DEF \
    (0)  // 输出数据格式 默认：0         可选参数为：[0-3] 0：RGB565
         // 1：RGB565(字节交换) 2：YUV422(YUYV) 3：YUV422(UYVY)
#define SCC8660_MANUAL_WB_DEF \
    (0)  // 手动白平衡   默认：0         可选参数为：[0,0x65-0xa0]
         // 0：关闭手动白平衡，启用自动白平衡    其他：手动白平衡 手动白平衡时
         // 参数范围0x65-0xa0
//================================================定义 SCC8660
// 参数配置（仅支持凌瞳高帧率版本）===========================
