...
extern uint8
    camera_send_image_frame_header[4];  // 声明摄像头数据发送到上位机的帧头
//=================================================摄像头公共库
// 基本配置================================================

//=================================================摄像头公共库
// 基础函数================================================
void camera_binary_image_decompression(
    const uint8* data1,
    uint8* data2,
    uint32 image_size);  // 摄像头二进制图像数据解压为十六进制八位数据 小钻风用
void camera_send_image(uart_index_enum uartn,
                       const uint8* image_addr,
                       uint32 image_size);  // 摄像头图像发送至上位机查看图像
void camera_fifo_init(void);                // 摄像头串口 FIFO 初始化
uint8 camera_init(uint8* source_addr,
                  uint8* destination_addr,
                  uint32 image_size);  // 摄像头采集初始化
uint8 camera_init_2(uint8* source_addr,
                    uint8* destination_addr,
                    uint32 image_size);  // 摄像头2采集初始化
//=================================================摄像头公共库
// 基础函数================================================
