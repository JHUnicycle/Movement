 ...
 * 2023-05-06       pudding            IIC延时640 → 1000
 *现在可以正常采集最老版本的IIC小钻风了
 ********************************************************************************************************************/
/*********************************************************************************************************************
 * 接线定义：
 *                  ------------------------------------
...
#define OV7725_COF_UART_TX \
    (UART1_RX_P02_3)  // 小钻风 UART-TX 引脚 要接在单片机 RX 上
#define OV7725_COF_UART_RX \
    (UART1_TX_P02_2)  // 小钻风 UART-RX 引脚 要接在单片机 TX 上

#define OV7725_COF_IIC_DELAY (1000)  // 小钻风 IIC 延时
...
#define OV7725_DATA_PIN \
    (P00_0)  // 数据引脚 这里是 只能是 GPIOx0 或者 GPIOx8 开始 连续八个引脚例如
             // F0-F7
...
#define OV7725_IMAGE_SIZE \
    (OV7725_W * OV7725_H / 8)  // 整体图像大小 OV7725_IMAGE_SIZE 不能超过 65535

#define OV7725_CONTRAST_DEF \
    (0x30)  // 阈值设置 摄像头二值化阈值 过大和过小的数值会被摄像头自动修正
#define OV7725_FPS_DEF \
    (50)  // 帧率设置 最高 150 帧 但最小分辨率才能达到最高帧率
//================================================定义 OV7725
//参数配置================================================
